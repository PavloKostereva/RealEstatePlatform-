...
export function MessagesList({ userId }: MessagesListProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      {messages.length === 0 ? (
        <p className="text-gray-500 text-center py-8">У вас поки що немає повідомлень</p>
      ) : (
                ...
                <div>
                  ...
                  {message.sender && (
                    <p className="text-sm text-gray-600">
                      Від: {message.sender.name || message.sender.email || 'Невідомий'}
                    </p>
                  )}
                </div>
                <div className="text-right">
                  <span className="text-sm text-gray-500">
                    {new Date(message.createdAt || message.created_at).toLocaleDateString('uk-UA')}
                  </span>
                  {!message.read && (
